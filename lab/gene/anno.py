...
BIT_LEN = len(GENIC_REGION_LIST) - 1
...
def parse_genic_region_val(region_val):
    ...
    """
    assert 0 <= region_val < (2 ** BIT_LEN)  # the maximum bit length of the genic region value is 6
    ...
    if region_val == 0:
        ...
        for bit_pos in range(1, BIT_LEN + 1):
            bit = int(region_val / (2 ** (BIT_LEN - bit_pos))) % 2
